...
Before a batch, choose condition, language, foil, storage location, and optionally a target deck or Box Mode set. Those defaults apply to every accepted card and are preserved with uncertain captures in Review.
...
Market prices are refreshed from Scryfall in a throttled background task every 12 hours and never block scanning. Set `PRICE_REFRESH_HOURS` to a different interval when deploying; 12–24 hours is recommended. A refresh can also be started with `POST /api/prices/refresh`; progress is available from `GET /api/prices/status`. Collection value is quantity-aware and appears beside the unique-entry count.
