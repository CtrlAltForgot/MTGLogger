...
def _prototype_ability(card:dict)->dict|None:
    match=re.search(r"(?:^|\n)Prototype\s*((?:\{[^}]+\})+)\s*[—-]\s*(\d+)\s*/\s*(\d+)",card.get("oracle_text") or "",re.IGNORECASE)
    return {"mana_cost":match.group(1).upper(),"power":match.group(2),"toughness":match.group(3)} if match else None


def _prototype_colors(mana_cost:str)->list[str]:
    return sorted({symbol for symbol in _mana_symbols({"mana_cost":mana_cost}) if symbol in "WUBRG"},key="WUBRG".index)


def _prototype_mana_value(mana_cost:str)->int:
    return sum(int(symbol) if symbol.isdigit() else 0 if symbol.upper()=="X" else 1 for symbol in _mana_symbols({"mana_cost":mana_cost}))


def _restore_prototype(card:dict)->None:
    original=card.pop("prototype_original",None)
    if not original:return
    for key,value in original.items():
        if value is None:card.pop(key,None)
        else:card[key]=value
    card.pop("prototyped",None)
...
def _squad_cost(card:dict)->str|None:
    match=re.search(r"(?:^|\n)Squad\s+((?:\{[^}]+\})+)",card.get("oracle_text") or "",re.IGNORECASE)
    return match.group(1).upper() if match else None
...
def _blitz_ability(card:dict)->dict|None:
    line=next((line.strip() for line in (card.get("oracle_text") or "").splitlines() if re.match(r"^Blitz(?:\s|[—-])",line.strip(),re.IGNORECASE)),None)
    if not line:return None
...
def _target_kind(card: dict) -> str | None:
    ...
    if re.search(r"target double-faced artifact",text):return "artifact"
    ...
    for player in state["players"]:
        ...
        for permanent in player["battlefield"]:
            if kind in {"any", "permanent"} or (kind=="player_or_planeswalker" and "Planeswalker" in permanent.get("type_line","")) or (kind=="creature_or_planeswalker" and any(value in permanent.get("type_line","") for value in ("Creature","Planeswalker"))) or (kind=="artifact_or_enchantment" and any(value in permanent.get("type_line","") for value in ("Artifact","Enchantment"))) or (kind=="artifact_or_creature" and any(value in permanent.get("type_line","") for value in ("Artifact","Creature"))) or (kind=="creature_or_spell" and "Creature" in permanent.get("type_line","")) or (kind=="creature_or_vehicle" and any(value in permanent.get("type_line","") for value in ("Creature","Vehicle"))) or (kind in {"creature","artifact","enchantment","land","planeswalker"} and kind in permanent.get("type_line", "").casefold()):
                ...
                if "double-faced artifact" in text and not ("Artifact" in permanent.get("type_line","") and len(permanent.get("card_faces") or [])>=2):continue
...
def legal_actions(state: dict, player_id: str, allow_direct_resolution:bool=True) -> list[dict]:
    ...
    if pending_miracle:
        ...
        if candidate:
            ...
            if _can_pay(player,cost_card,x_value=0) or _has_x_cost(cost_card) and action.get("x_max",-1)>=0:
                cast_actions=[]
                if variants:
                    cast_actions.extend({**action,"target_steps":steps,"target_count":len(steps),"allow_zero_targets":not steps} for steps in variants)
                elif not required or targets:cast_actions.append({**action,**({"targets":targets} if targets else {})})
                squad_cost=_squad_cost(candidate)
                if squad_cost:
                    expanded=list(cast_actions)
                    for base_action in cast_actions:
                        for count in range(1,21):
                            squad_card={**cost_card,"mana_cost":f"{cost_card.get('mana_cost') or ''}{squad_cost*count}"}
                            if not _can_pay(player,squad_card,x_value=0):break
                            expanded.append({**base_action,"squad_count":count,"squad_cost":squad_cost,"label":f"{base_action['label']} + Squad {squad_cost} ×{count}"})
                    cast_actions=expanded
    ...
    blitz_sources=[*((card,"blitz_hand") for card in player["hand"])]
    blitz_sources.extend((card,"blitz_graveyard") for card in player["graveyard"] if "you may cast this card from your graveyard using its blitz ability" in (card.get("oracle_text") or "").casefold())
    for blitz_card,blitz_source in blitz_sources:
        blitz=_blitz_ability(blitz_card);discard_options=[candidate["instance_id"] for candidate in player["hand"] if candidate is not blitz_card]
        if not blitz or not active or not main or state["stack"] or player["life"]<blitz["life_cost"] or blitz["discard_count"] and not discard_options or not _can_pay(player,{**blitz_card,"mana_cost":blitz["mana_cost"]}):continue
        label=f"Blitz {blitz_card['name']} · {blitz['mana_cost']}"+(" · discard a card" if blitz["discard_count"] else "")+(f" · pay {blitz['life_cost']} life" if blitz["life_cost"] else "")
        actions.append({"type":"cast","card_id":blitz_card["instance_id"],"source":blitz_source,"blitzed":True,"mana_cost":blitz["mana_cost"],"cost_kind":"discard" if blitz["discard_count"] else None,"cost_amount":blitz["discard_count"],"cost_options":discard_options if blitz["discard_count"] else [],"life_cost":blitz["life_cost"],"label":label})
    alternate_casts=[]
    for grave_card in player["graveyard"]:
    ...
    for card, source in castable:
        ...
        prototype=_prototype_ability(card) if source in {"hand","command","graveyard_permission","exile_permission"} else None
        if prototype and ((active and main and not state["stack"]) or instant_speed):
            prototype_card={**card,"mana_cost":prototype["mana_cost"]}
            if _can_pay(player,prototype_card,generic_adjustment):actions.append({"type":"cast","card_id":card["instance_id"],"source":source,"prototyped":True,"mana_cost":prototype["mana_cost"],"commander_tax":total_tax,"affinity_reduction":affinity_reduction,"delirium_reduction":delirium_reduction,"label":f"Prototype {card['name']} as {prototype['power']}/{prototype['toughness']} · {prototype['mana_cost']}"})
        ...
        squad_cost=_squad_cost(card) if source in {"hand","command"} else None
        if squad_cost:
            for count in range(1,21):
                repeated_cost={**cost_card,"mana_cost":f"{cost_card.get('mana_cost') or ''}{squad_cost*count}"}
                if not _can_pay(player,repeated_cost,generic_adjustment,x_value=0):break
                actions.append({**action,"squad_count":count,"squad_cost":squad_cost,"label":f"{action['label']} + Squad {squad_cost} ×{count}"})
...
def _resolve_spell(state: dict) -> None:
    ...
    if item.get("kind")=="end_combat_sacrifice_trigger":
        tokens=[token for owner in state["players"] for token in list(owner["battlefield"]) if token["instance_id"] in set(item.get("token_ids",[]))]
        for token in tokens:_sacrifice_permanents(state,next(owner for owner in state["players"] if token in owner["battlefield"]),[token])
        _log(state,f"{len(tokens)} temporary attacking token(s) were sacrificed at end of combat.");return
    ...
    if item.get("kind")=="dash_return_trigger":
        ...
        return
    if item.get("kind")=="blitz_sacrifice_trigger":
        permanent=next((permanent for owner in state["players"] for permanent in owner["battlefield"] if permanent["instance_id"]==item.get("source_id") and permanent.get("blitzed")),None)
        if permanent:
            owner=next(owner for owner in state["players"] if permanent in owner["battlefield"]);_sacrifice_permanents(state,owner,[permanent]);_log(state,f"{permanent['name']} was sacrificed at the end step after being blitzed.")
        return
    ...
    elif source_permanent and "transform up to one other target double-faced artifact you control" in effect_text:
        transform_target=target or next((permanent for owner in state["players"] for permanent in owner["battlefield"] if valid_multi_ids and permanent["instance_id"]==valid_multi_ids[0]),None)
        if transform_target and transform_target is not source_permanent:_transform(state,transform_target)
        effect_text=re.sub(r"transform up to one other target double-faced artifact you control\.?\s*","",effect_text,flags=re.IGNORECASE)
    ...
    adventure_sacrifice=re.search(r"(you may )?sacrifice (an artifact or creature|a non-demon creature)\.\s*if you do,\s*(.+)",effect_text,re.DOTALL)
    if adventure_sacrifice:
        optional=bool(adventure_sacrifice.group(1));kind=adventure_sacrifice.group(2).casefold();choices=[candidate["instance_id"] for candidate in caster["battlefield"] if (kind=="an artifact or creature" and any(card_type in candidate.get("type_line","") for card_type in ("Artifact","Creature"))) or (kind=="a non-demon creature" and "Creature" in candidate.get("type_line","") and not re.search(r"\bDemon\b",candidate.get("type_line",""),re.IGNORECASE))]
        if choices:
            source_name=(source_permanent or card).get("name",card["name"]);state["pending_zone_choice"]={"player_id":caster["id"],"source_name":source_name,"source_id":item.get("source_id"),"zone":"battlefield","destination":"graveyard","card_ids":choices,"optional":optional,"sacrifice":True,"continuation":adventure_sacrifice.group(3).strip()};state["priority_player_id"]=caster["id"];_log(state,f"{caster['name']} {'may' if optional else 'must'} sacrifice {kind} for {source_name}.")
        else:_log(state,f"{caster['name']} had no {kind} to sacrifice for {card['name']}.")
        return
    ...
    if "create a token that's a copy of this creature" in effect_text and source_permanent and "Creature" in source_permanent.get("type_line",""):
        tokens=[]
        for _ in range(max(1,int(item.get("squad_count") or 1))):
            ...
            for key in ("attached_to","attachment_keywords","attachment_rules","temporary_power","temporary_toughness","temporary_keywords","temporary_backup_rules","deathtouch_damage","activated_ability_usage","entered_turn","squad_count"):token.pop(key,None)
            tokens.append(token)
        _enter_battlefield(state,caster,tokens,"token");_log(state,f"{caster['name']} created {len(tokens)} token {'copies' if len(tokens)!=1 else 'copy'} of {source_permanent['name']}.");return
    ...
    exile_top_two=re.search(r"(?:look at |exile )?the top two cards of your library and exile them face down|exile the top two cards of your library",effect_text)
    if exile_top_two:
        exiled=[]
        for _ in range(min(2,len(caster["library"]))):exiled_card=caster["library"].pop();_put_into_exile(state,caster,[exiled_card],"library",caster["id"]);exiled.append(exiled_card)
        wizard_permission="if you control a wizard" in effect_text
        expires=state["turn"] if state.get("active_player_id")==caster["id"] else state["turn"]+1
        for exiled_card in exiled:
            if wizard_permission:exiled_card["exile_play_while_wizard"]=True
            else:
                exiled_card["exile_play_until_turn"]=expires
                if "Land" not in exiled_card.get("type_line",""):exiled_card["exile_cast_until_turn"]=expires
        _log(state,f"{caster['name']} exiled {len(exiled)} card(s) and may play them {'while they control a Wizard' if wizard_permission else 'until their next end step'}.")
    ...
    lasting_fayth=card.get("name")=="Lasting Fayth" and "hero creature token" in effect_text and "for each land you control" in effect_text
    if lasting_fayth:
        token={"instance_id":_id(),"scryfall_id":"token-hero","name":"Hero Token","image_url":None,"type_line":"Token Creature — Hero","oracle_text":"","mana_cost":"","mana_value":0,"colors":[],"power":"1","toughness":"1","owner_id":caster["id"],"controller_id":caster["id"],"tapped":False,"damage":0,"counters":{},"summoning_sick":True,"token":True,"keywords":[]};_enter_battlefield(state,caster,[token],"token");land_count=sum("Land" in permanent.get("type_line","") for permanent in caster["battlefield"]);_add_counters(state,token,"+1/+1",land_count,caster["id"],"effect");_log(state,f"{caster['name']} created a Hero Token with {land_count} +1/+1 counter(s).");effect_text=""
    ...
    if token_match:
        ...
        for _ in range(amount):
            ...
            if end_combat_sacrifice:token["sacrifice_end_combat"]=True
    ...
    if is_permanent_spell and rebound_from_hand:
        ...
        card["squad_count"]=int(item.get("squad_count") or 0)
        ...
        if item.get("blitzed"):card["blitzed"]=True;card["blitz_controller_id"]=caster["id"];card["blitz_sacrifice_turn"]=state["turn"];card.setdefault("temporary_keywords",[]).append("Haste")
        ...
        if card["squad_count"]:
            ability={"name":f"{card['name']} — Squad","oracle_text":"Create a token that's a copy of this creature.","type_line":"Ability","mana_cost":""};state["stack"].append({"id":_id(),"kind":"squad_trigger","card":ability,"controller_id":caster["id"],"target_id":None,"source_id":card["instance_id"],"squad_count":card["squad_count"]});_log(state,f"{card['name']}'s Squad ability triggered for {card['squad_count']} token {'copies' if card['squad_count']!=1 else 'copy'}.")
...
def _leave_battlefield(state: dict, owner: dict, card: dict, destination: str, trigger_sources:list[tuple[dict,dict]]|None=None, trigger_dedupe:set[str]|None=None, exile_actor_id:str|None=None, exile_batch_size:int|None=None) -> None:
    ...
    if blitzed:
        ability={"name":f"{card['name']} — Blitz draw","type_line":"Ability","mana_cost":"","oracle_text":"Draw a card."};state["stack"].append({"id":_id(),"kind":"trigger","card":ability,"controller_id":blitz_controller,"target_id":None});_log(state,f"{card['name']}'s blitz death draw triggered.")
...
def _queue_triggers(state: dict, event: str, event_card: dict | None, event_owner: dict, dedupe:set[str]|None=None, sources_override:list[tuple[dict,dict]]|None=None) -> None:
    ...
    for owner, source in sources:
        ...
        attachment_triggers=[]
        for rules in source.get("attachment_rules",{}).values():
            attachment_triggers.extend(re.findall(r'"([^\"]*(?:when(?:ever)?|at the beginning)[^\"]*)"',rules,re.IGNORECASE))
            if re.match(r"\s*(?:when(?:ever)?|at the beginning)\b",rules,re.IGNORECASE):attachment_triggers.append(rules)
        text = "\n".join([_active_level_text(source),*(source.get("temporary_backup_rules") or []),*attachment_triggers])
        ...
        for clause in clauses:
            ...
            elif event=="attackers_declared" and "another target creature attacking the same player or planeswalker" in effect.casefold():
                source_id=source.get("instance_id");defender_id=state.get("combat",{}).get("attack_targets",{}).get(source_id);attacking_ids=set(state.get("combat",{}).get("attackers",[]));special_targets=[{"id":candidate["instance_id"],"name":candidate["name"],"kind":"permanent","controller_id":candidate["controller_id"]} for candidate in owner["battlefield"] if candidate["instance_id"]!=source_id and candidate["instance_id"] in attacking_ids and state["combat"]["attack_targets"].get(candidate["instance_id"])==defender_id and "Creature" in candidate.get("type_line","")]
            elif event=="attackers_declared" and "target creature defending player controls" in effect.casefold():
                defender_id=state.get("combat",{}).get("attack_targets",{}).get(source.get("instance_id"));defender=next((candidate for candidate in state["players"] if candidate["id"]==defender_id),None);special_targets=[{"id":candidate["instance_id"],"name":candidate["name"],"kind":"permanent","controller_id":defender["id"]} for candidate in (defender or {}).get("battlefield",[]) if "Creature" in candidate.get("type_line","")]
            ...
            elif "transform up to one other target double-faced artifact you control" in effect.casefold():
                candidates=[{"id":candidate["instance_id"],"name":candidate["name"],"kind":"permanent","controller_id":owner["id"]} for candidate in owner["battlefield"] if candidate["instance_id"]!=source.get("instance_id") and "Artifact" in candidate.get("type_line","") and len(candidate.get("card_faces") or [])>=2];dynamic_steps=[{"label":"Choose a double-faced artifact (or finish)","targets":candidates,"distinct":True}];dynamic_min=0
...
def _advance_turn_phase(state: dict) -> None:
    ...
    if index == len(PHASES) - 1:
        ...
        if state["phase"] == "ending":
            ...
            for permanent in [card for owner in state["players"] for card in owner["battlefield"] if card.get("blitz_sacrifice_turn")==state["turn"]]:
                permanent.pop("blitz_sacrifice_turn",None);controller=_player(state,permanent.get("blitz_controller_id",permanent["controller_id"]));ability_card={"name":f"{permanent['name']} — Blitz sacrifice","type_line":"Ability","mana_cost":"","oracle_text":f"Sacrifice {permanent['name']}."};state["stack"].append({"id":_id(),"kind":"blitz_sacrifice_trigger","card":ability_card,"controller_id":controller["id"],"source_id":permanent["instance_id"]});_log(state,f"{permanent['name']}'s blitz sacrifice trigger was put on the stack.")
...
def perform_action(state: dict, player_id: str, action: dict, allow_direct_resolution:bool=True) -> dict:
    ...
    if action_type in {"sticktwister_discard","sticktwister_sacrifice","sticktwister_take_damage"}:
        ...
        if action_type=="decline_miracle":
            ...
            squad_count=max(0,int(action.get("squad_count") or 0));squad_cost=_squad_cost(candidate);base_cost_card={**candidate,"mana_cost":pending["mana_cost"]}
            if squad_count and not squad_cost:raise RuleViolation("That Miracle spell has no Squad cost")
            cost_card={**base_cost_card,"mana_cost":f"{base_cost_card.get('mana_cost') or ''}{(squad_cost or '')*squad_count}"};x_value=int(action.get("x_value") or 0);has_x=_has_x_cost(cost_card);maximum=_maximum_x(player,cost_card)
            ...
            _pay_mana(state,player,cost_card,x_value=x_value);player["hand"].remove(candidate);state["pending_miracle"]=(state.get("pending_miracle_queue") or []).pop(0) if state.get("pending_miracle_queue") else None;stack_item={"id":_id(),"kind":"spell","card":candidate,"controller_id":player_id,"target_id":target_id,"target_ids":target_ids,"mode_indices":[],"mode_targets":[],"x_value":x_value,"squad_count":squad_count,"miracle_cast":True,"cast_source_zone":"hand"};state["stack"].append(stack_item);_record_spell_cast(state,player);candidate["cast_source_zone"]="hand";_queue_triggers(state,"cast",candidate,player);_queue_cascade_triggers(state,player,candidate);_queue_storm_trigger(state,player,candidate,stack_item);candidate.pop("cast_source_zone",None);[_queue_ward(state,player,target,stack_item) for target in [target_id,*target_ids] if target];state["consecutive_passes"]=0;state["pending_phase_advance"]=False
            ...
            _log(state,f"{player['name']} cast {candidate['name']} for its Miracle cost {pending['mana_cost']}{f' with X={x_value}' if has_x else ''}{f' and paid Squad {squad_cost} ×{squad_count}' if squad_count else ''}.")
        ...
        requested_source=action.get("source");zone_name="graveyard" if requested_source in {"flashback","escape","mutate_graveyard","graveyard_permission","blitz_graveyard"} else "exile" if requested_source in {"airbend","suspend","foretell","plot","rebound","exile_permission"} else "hand" if requested_source in {"mutate_hand","evoke","dash_hand","blitz_hand","bestow"} else "command" if requested_source in {"mutate_command","dash_command"} else requested_source if requested_source in {"hand","command"} else next((zone for zone in ("hand","command") if any(card["instance_id"]==action.get("card_id") for card in player.get(zone,[]))),None)
        source=requested_source if requested_source in {"flashback","escape","mutate_graveyard","graveyard_permission","blitz_graveyard","airbend","suspend","foretell","plot","rebound","exile_permission","mutate_hand","mutate_command","evoke","dash_hand","dash_command","blitz_hand","bestow"} else zone_name;card=next((card for card in player.get(zone_name or "hand",[]) if card["instance_id"]==action.get("card_id")),None);flashback=_flashback_ability(card or {}) if source=="flashback" else None;escape=_escape_ability(card or {}) if source=="escape" else None
        requested_prototyped=bool(action.get("prototyped"));requested_kicked=bool(action.get("kicked"));requested_multikicker=max(0,int(action.get("multikicker_count") or 0));requested_squad=max(0,int(action.get("squad_count") or 0));requested_entwined=bool(action.get("entwined"));requested_buyback=bool(action.get("buyback"));requested_convoke=bool(action.get("convoke"));requested_waterbend=bool(action.get("waterbend"));requested_overloaded=bool(action.get("overloaded"));requested_evoked=requested_source=="evoke";requested_dashed=requested_source in {"dash_hand","dash_command"};requested_blitzed=requested_source in {"blitz_hand","blitz_graveyard"};requested_bestowing=bool(action.get("bestowing") or requested_source=="bestow");requested_delve=bool(action.get("delve"));requested_mutating=bool(action.get("mutating") or requested_source in {"mutate_hand","mutate_graveyard","mutate_command"});requested_blight=bool(action.get("blighted") or (action.get("cost_card_ids") and _optional_blight_cost(card or {})));requested_blessing_top=bool(action.get("blessing_top"));requested_target_count=len(action.get("target_ids") or []);available=next((entry for entry in legal_actions(state,player_id,allow_direct_resolution) if entry["type"]=="cast" and entry["card_id"]==action.get("card_id") and entry.get("source")==source and bool(entry.get("prototyped"))==requested_prototyped and int(entry.get("multikicker_count") or 0)==requested_multikicker and int(entry.get("squad_count") or 0)==requested_squad and bool(entry.get("bestowing"))==requested_bestowing and bool(entry.get("entwined"))==requested_entwined and bool(entry.get("overloaded"))==requested_overloaded and bool(entry.get("evoked"))==requested_evoked and bool(entry.get("dashed"))==requested_dashed and bool(entry.get("blitzed"))==requested_blitzed and bool(entry.get("delve"))==requested_delve and (not requested_delve or entry.get("cost_amount")==len(action.get("cost_card_ids") or [])) and bool(entry.get("mutating"))==requested_mutating and entry.get("mutate_position")==action.get("mutate_position") and bool(entry.get("kicked"))==requested_kicked and bool(entry.get("buyback"))==requested_buyback and bool(entry.get("convoke"))==requested_convoke and bool(entry.get("waterbend"))==requested_waterbend and bool(entry.get("blighted"))==requested_blight and bool(entry.get("blessing_top"))==requested_blessing_top and (entry.get("target_count") is None or entry["target_count"]==requested_target_count)),None)
        ...
        if requested_blitzed:
            blitz=_blitz_ability(card) or {};life_cost=int(blitz.get("life_cost") or 0)
            if player["life"]<life_cost:raise RuleViolation("You cannot pay the blitz life cost")
            player["life"]-=life_cost
            if blitz.get("discard_count"):
                discarded=[candidate for candidate in player["hand"] if candidate is not card and candidate["instance_id"] in set(selected_cost_ids)]
                if len(discarded)!=1:raise RuleViolation("Choose the required card to discard for blitz")
                _discard_cards(state,player,discarded)
        ...
        if requested_prototyped:
            prototype=_prototype_ability(card)
            if not prototype:raise RuleViolation("That card no longer has Prototype")
            card["prototype_original"]={key:deepcopy(card.get(key)) for key in ("mana_cost","mana_value","power","toughness","colors")};card.update({"mana_cost":prototype["mana_cost"],"mana_value":_prototype_mana_value(prototype["mana_cost"]),"power":prototype["power"],"toughness":prototype["toughness"],"colors":_prototype_colors(prototype["mana_cost"]),"prototyped":True})
        if card.get("commander"): player["commander_casts"] = player.get("commander_casts", 0) + 1
        effective_target=target_id or (mode_targets[0] if len(mode_targets)==1 else None);stack_item={"id": _id(), "card": card, "controller_id": player_id, "target_id": effective_target,"target_ids":target_ids,"mode_indices":chosen_modes,"mode_targets":mode_targets,"x_value":x_value,"allow_zero_targets":bool(available.get("allow_zero_targets")),"flashback":bool(flashback),"buyback":requested_buyback,"rebound_cast":source=="rebound","escaped":bool(escape),"escape_counters":escape["counters"] if escape else 0,"escape_counter_choice":escape["counter_choice"] if escape else False,"suspended_cast":source=="suspend","prototyped":requested_prototyped,"kicked":requested_kicked,"multikicker_count":requested_multikicker,"squad_count":requested_squad,"entwined":requested_entwined,"overloaded":requested_overloaded,"blighted":requested_blight,"blessing_top":requested_blessing_top,"evoked":requested_evoked,"dashed":requested_dashed,"blitzed":requested_blitzed,"bestowing":requested_bestowing,"delved_cards":delved_cards if requested_delve else [],"mutating":requested_mutating,"mutate_position":action.get("mutate_position"),"cast_source_zone":"graveyard" if source in {"flashback","escape","mutate_graveyard","graveyard_permission","blitz_graveyard"} else "exile" if source in {"airbend","suspend","foretell","plot","rebound","exile_permission"} else "hand" if source in {"mutate_hand","evoke","dash_hand","blitz_hand","bestow"} else "command" if source in {"mutate_command","dash_command"} else source};state["stack"].append(stack_item);state["stack"].extend(cost_triggers); state["consecutive_passes"] = 0; state["pending_phase_advance"] = False
        if requested_waterbend:_queue_triggers(state,"waterbend",card,player)
        _record_spell_cast(state,player);card["cast_source_zone"]="graveyard" if source in {"flashback","escape","mutate_graveyard","graveyard_permission","blitz_graveyard"} else "exile" if source in {"airbend","suspend","foretell","plot","exile_permission"} else "hand" if source in {"mutate_hand","blitz_hand"} else "command" if source=="mutate_command" else source
        ...
        _log(state, f"{player['name']} cast {card['name']}{' using blitz from the graveyard' if source=='blitz_graveyard' else ' using blitz' if source=='blitz_hand' else ' from foretell' if source=='foretell' else ' from suspend without paying its mana cost' if source=='suspend' else ' using flashback' if flashback else ' using airbend' if source=='airbend' else ''}{' with kicker' if requested_kicked else ''}{' using waterbend' if requested_waterbend else ''}{' using convoke' if requested_convoke else ''}{' after blighting' if requested_blight else ''}{f' with X={x_value}' if has_x else ''}{f' choosing {mode_label}' if mode_label else ''}{f' with {tax} commander tax' if tax else ''}{f' and {affinity_reduction} affinity reduction' if affinity_reduction else ''}{f' and {delirium_reduction} Delirium reduction' if delirium_reduction else ''}{f' by beholding {', '.join(behold_names)}' if available.get('cost_kind')=='behold' else ''}{' targeting '+next((target['name'] for target in targets if target['id']==target_id),'') if target_id else ''}.")
