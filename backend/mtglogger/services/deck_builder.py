...
def synergy_tags(card:BuildCandidate)->set[str]:
    text=f"{card.type_line} {card.oracle_text} {' '.join(card.keywords)}".casefold()
    tags={theme for theme in THEMES if theme_hits(card,theme)}
    if "—" in card.type_line:
        subtypes=re.split(r"\s+",card.type_line.split("—",1)[1].casefold())
    ...
    if "legendary" in card.type_line.casefold() or "legendary" in text:tags.add("legends")
...
def build_deck(candidates:list[BuildCandidate],format_name:str,colors:list[str],strategy:str="balanced",name:str="Auto-built deck",focus:str="",focus_mode:str="prefer")->dict:
    ...
    theme=choose_theme(focused if focused else spells,strategy)
    selected:dict[str,dict]={};used=Counter();warnings:list[str]=[]
    ...
    def dynamic_score(card:BuildCandidate)->tuple[float,list[str]]:
        ...
        pairings=sorted(((pair_synergy(card,other),other.name) for other in partners if card_key(card)!=card_key(other)),reverse=True)
    ...
    pair_scores=[pair_synergy(left,right) for index,left in enumerate(nonlands) for right in nonlands[index+1:]]
    ...
    for entry in sorted(selected.values(),key=lambda item:(item["role"]!="leader",item["role"]!="land",item["card"].mana_value,item["card"].name)):
        ...
        partners=sorted(((pair_synergy(card,other),other.name) for other in nonlands if card_key(card)!=card_key(other)),reverse=True)
