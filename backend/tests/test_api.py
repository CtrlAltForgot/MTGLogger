...
def test_current_footer_ocr_reads_only_two_modern_identity_rows():
    ...
    class Ocr:
        ...
        def predict(self, rows):
            self.calls.append(rows)
            return [Result("Plains"), Result("L0282"), Result("TLA · EN")]
    ...
    assert len(recognizer._footer_ocr.calls) == 1
    assert len(recognizer._footer_ocr.calls[0]) == 3
...
def test_ocr_hints_reads_rarity_prefixed_current_collector_number():
    ...
    noisy_title, noisy_number, noisy_set, _ = CardRecognizer.hints(
        "COOO3\nTLAEN金田栄路/UI KANDA\n©2025 Wizards"
    )
