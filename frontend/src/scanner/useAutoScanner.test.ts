...
describe('bounded recognition pipeline',()=>{
  ...
  it('uploads a focused portrait crop for a normally detected card',()=>{
    ...
    expect(crop!.height).toBeLessThanOrEqual(720)
    expect(crop!.x).toBeGreaterThan(0)
    expect(crop!.width/crop!.height).toBeCloseTo(63/88,2)
  })

  it('expands an internal artwork or rules-box detection to card proportions',()=>{
