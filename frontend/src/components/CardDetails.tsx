...
export function CardDetailsProvider({children}:{children:ReactNode}){
  ...
  return <Context.Provider value={setId}>{children}<Dialog open={!!id} onClose={()=>setId(undefined)} fullWidth maxWidth="md"><DialogTitle className={details?'card-name':undefined} sx={{pr:7}}>{details?.name||'Card details'}<IconButton onClick={()=>setId(undefined)} sx={{position:'absolute',right:12,top:10}}><Close/></IconButton></DialogTitle><DialogContent className="card-detail-body" dividers sx={{minHeight:460}}>{error?<Typography color="error.main">{error}</Typography>:!details?<Box minHeight={400} display="grid" sx={{placeItems:'center'}}><CircularProgress/></Box>:<Grid container spacing={3}><Grid size={{xs:12,sm:5}}>{details.image_url&&<Box component="img" src={artworkUrl(details.image_url)} alt={details.name} sx={{display:'block',width:'100%',maxWidth:360,mx:'auto',borderRadius:3,boxShadow:'0 18px 50px rgba(0,0,0,.35)'}}/>}</Grid><Grid size={{xs:12,sm:7}}><Typography className="card-title" variant="h4">{details.name}</Typography><Typography className="card-printing" color="text.secondary">{details.set_name} ({details.set_code.toUpperCase()}) #{details.collector_number} · {details.language?.toUpperCase()}</Typography><Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap mt={2}>{details.rarity&&<Chip label={details.rarity}/>} {details.finishes?.map(finish=><Chip key={finish} variant="outlined" label={finish}/>)}</Stack><Divider sx={{my:2}}/><Typography fontWeight={800}>{details.type_line}</Typography>{rules&&<Typography whiteSpace="pre-line" mt={2}>{rules}</Typography>}{(details.power||details.toughness)&&<Stack direction="row" alignItems="center" spacing={1.25} mt={2} aria-label={`Power ${details.power}, toughness ${details.toughness}`}><Stack direction="row" alignItems="center" spacing={.5}><SportsMma fontSize="small" color="primary"/><Typography fontWeight={900}>{details.power}</Typography></Stack><Typography color="text.secondary">—</Typography><Stack direction="row" alignItems="center" spacing={.5}><Security fontSize="small" color="primary"/><Typography fontWeight={900}>{details.toughness}</Typography></Stack></Stack>}{details.loyalty&&<Typography fontWeight={900} mt={2}>Loyalty {details.loyalty}</Typography>}{details.flavor_text&&<Typography color="text.secondary" fontStyle="italic" mt={2}>{details.flavor_text}</Typography>}<Divider sx={{my:2}}/><Stack direction="row" spacing={3}><Box><Typography variant="caption" color="text.secondary">NONFOIL</Typography><Typography className="card-price" variant="h6">{price?`$${price}`:'—'}</Typography></Box><Box><Typography variant="caption" color="text.secondary">FOIL</Typography><Typography className="card-price" variant="h6">{foilPrice?`$${foilPrice}`:'—'}</Typography></Box></Stack>{details.artist&&<Typography color="text.secondary" mt={2}>Illustrated by {details.artist}</Typography>}{legal.length>0&&<><Typography fontWeight={800} mt={2} mb={1}>Legal in</Typography><Stack direction="row" gap={.75} flexWrap="wrap">{legal.map(format=><Chip key={format} size="small" label={format}/>)}</Stack></>}{details.scryfall_uri&&<Button href={details.scryfall_uri} target="_blank" rel="noreferrer" startIcon={<OpenInNew/>} sx={{mt:2}}>Open exact printing on Scryfall</Button>}</Grid></Grid>}</DialogContent></Dialog></Context.Provider>
}
