...
export default function Database(){
  ...
  return <>
    <Stack direction={{xs:'column',md:'row'}} justifyContent="space-between" spacing={2} mb={3}>
      ...
      <Stack direction="row" spacing={1} alignItems="center"><Chip color={status?.state==='running'?'primary':status?.state==='failed'?'error':status?.coverage_percent===100?'success':'warning'} label={status?.state==='running'?'Updating live':status?.state==='failed'?'Update interrupted':status?.coverage_percent===100?'Up to date':'Incomplete'}/>{activeCode&&<Chip variant="outlined" label={`Current · ${activeCode.toUpperCase()}`}/>}</Stack>
    </Stack>
