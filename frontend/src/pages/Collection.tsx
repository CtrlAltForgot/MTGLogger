...
export default function Collection(){
  ...
  return <>
    ...
    <Box className="collection-grid" mt={3}>{items.map(item=><Card key={item.id} sx={{display:'flex',overflow:'hidden',position:'relative',alignItems:'flex-start',minHeight:202}}>
      ...
      <Box py={1.5} pl={.5} pr={7} minWidth={0}><Typography className="card-title" fontWeight={800} noWrap>{item.card_name}</Typography><Typography className="card-printing" color="text.secondary" variant="body2">{item.set_name} #{item.collector_number}</Typography><PriceMovement item={item}/><Stack direction="row" spacing={.5} mt={1} flexWrap="wrap"><Chip size="small" label={`×${item.quantity}`}/><Chip size="small" variant="outlined" label={item.condition.replaceAll('_',' ')}/>{item.foil&&<Chip size="small" color="warning" label="Foil"/>}</Stack><Typography display="block" variant="caption" color="text.secondary">Deck · {item.deck_assignments.length?item.deck_assignments.map(deck=>`${deck.deck_name} ×${deck.quantity}`).join(', '):'None'}</Typography><Typography variant="caption" color="text.secondary">Storage · {item.storage_location}</Typography></Box>
