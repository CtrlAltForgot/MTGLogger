...
export default function Collection(){
  ...
  const closeDeckMode=(force=false)=>{if(busy&&!force)return;setDeckMode(false);setSelected(new Set());setTargetDeck(null)}
  ...
  const toggleAllVisible=()=>setSelected(current=>{const next=new Set(current);if(allVisibleSelected)selectableItems.forEach(item=>next.delete(item.id));else selectableItems.forEach(item=>next.add(item.id));return next})
  const addToDeck=async()=>{if(!targetDeck||!selectedItems.length)return;setBusy(true);setError(undefined);try{await request(`/decks/${targetDeck.id}/entries`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({entries:selectedItems.map(item=>({inventory_id:item.id,quantity:available(item)}))})});closeDeckMode(true);setReload(x=>x+1)}catch(e){setError(e instanceof Error?e.message:'Could not add cards to deck')}finally{setBusy(false)}}
