...
import {
  ...
  Skeleton, Snackbar, TablePagination, TextField, Tooltip, Typography,
} from '@mui/material'
...
import {saveEntry} from '../collection/saveEntry'
...
const languages=[['en','English'],['es','Spanish'],['fr','French'],['de','German'],['it','Italian'],['pt','Portuguese'],['ja','Japanese'],['ko','Korean'],['ru','Russian'],['zhs','Chinese (Simplified)'],['zht','Chinese (Traditional)'],['he','Hebrew'],['la','Latin'],['grc','Ancient Greek'],['ar','Arabic'],['sa','Sanskrit'],['phy','Phyrexian']]
...
const scannedAtFormatter=new Intl.DateTimeFormat(undefined,{dateStyle:'medium',timeStyle:'short'})

export const formatScannedAt=(value:string)=>scannedAtFormatter.format(new Date(value))
...
export default function Collection(){
  ...
  const editingOriginal=useRef<Inventory|null>(null)
  ...
  useEffect(()=>{let active=true;setLoading(true);const params=new URLSearchParams({q:query,page:String(page+1),page_size:String(pageSize),...Object.fromEntries(new URLSearchParams(sorts[sort]))});if(location)params.set('storage_location',location);void request<{items:Inventory[],total:number,total_cards:number,collection_value:number,page_size:number}>(`/inventory?${params}`).then(x=>{if(!active)return;setItems(x.items);setTotal(x.total);setTotalCards(x.total_cards);setCollectionValue(Number(x.collection_value));if(x.total>0&&page*x.page_size>=x.total)setPage(Math.max(0,Math.ceil(x.total/x.page_size)-1))}).catch(e=>{if(active)setError(e instanceof Error?e.message:'Could not load collection')}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[query,sort,location,page,pageSize,reload])
  const save=async()=>{
    if(!editing||!editingOriginal.current||saving.current)return
    saving.current=true;setBusy(true);setError(undefined)
    try{
      const saved=await saveEntry(editingOriginal.current,editing,selectedCopies.size,copyFoil,copyCondition)
      setItems(current=>current.map(item=>item.id===saved.id?{...item,...saved}:item))
      setEditing(null);setSavedNotice(true);setReload(value=>value+1)
    }catch(e){setError(e instanceof Error?e.message:'Could not save entry');setReload(value=>value+1)}
  ...
  const openEditor=(item:Inventory)=>{editingOriginal.current=item;setSelectedCopies(new Set());setCopyFoil(item.foil);setCopyCondition(item.condition);setEditing({...item})}
  ...
  return <>
    ...
    {loading&&items.length===0?<Box className="collection-grid" mt={3}>{Array.from({length:12},(_,index)=><Skeleton key={index} variant="rounded" height={202}/>)}</Box>:<Box className="collection-grid" mt={3}>{items.map(item=>{const remaining=available(item),checked=selected.has(item.id);return <Card key={item.id} onClick={()=>deckMode&&remaining>0&&toggleSelected(item)} sx={{display:'flex',overflow:'visible',position:'relative',alignItems:'flex-start',minHeight:202,cursor:deckMode&&remaining>0?'pointer':'default',outline:checked?'3px solid':'none',outlineColor:'primary.main',opacity:deckMode&&remaining===0?.48:1}}>
      {item.image_url?<FoilArtwork src={artworkUrl(item.image_url)} alt={item.card_name} foil={item.foil} sx={{width:{xs:116,sm:128},flexShrink:0,aspectRatio:'63 / 88',m:1,borderRadius:1.5,border:'1px solid',borderColor:'divider',bgcolor:'#080506'}} imageSx={{objectFit:'contain',objectPosition:'center'}}/>:<Box sx={{width:{xs:116,sm:128},flexShrink:0,aspectRatio:'63 / 88',m:1,bgcolor:'action.hover',border:'1px solid',borderColor:'divider',borderRadius:1.5}}/>}
      {item.quantity>1&&<Chip size="small" color="primary" label={`×${item.quantity}`} sx={{position:'absolute',left:0,top:0,transform:'translate(-38%,-38%)',zIndex:4,height:25,minWidth:34,fontWeight:900,boxShadow:'0 3px 12px rgba(0,0,0,.58)'}}/>}
    ...
    {!loading&&!items.length&&<Typography textAlign="center" color="text.secondary" mt={8}>No cards found. The scanner is ready when you are.</Typography>}
    ...
    <Dialog open={!!editing} onClose={()=>!busy&&setEditing(null)} fullWidth maxWidth="sm"><DialogTitle>Edit collection entry</DialogTitle>{editing&&<DialogContent>{error&&<Alert severity="error" sx={{mb:2}}>{error}</Alert>}<Box component="fieldset" disabled={busy} sx={{border:0,p:0,m:0,minWidth:0}}><Stack spacing={2} mt={1}>
      ...
      <CopySelector item={editing} selected={selectedCopies} onToggle={index=>{if(!busy)toggleCopy(index)}}/>
      ...
      <Stack direction={{xs:'column',sm:'row'}} spacing={2}><TextField select fullWidth label="Condition" value={editing.condition} onChange={e=>setEditing({...editing,condition:e.target.value})}>{conditions.map(([value,label])=><MenuItem key={value} value={value}>{label}</MenuItem>)}</TextField><TextField select fullWidth label="Language" value={editing.language} onChange={e=>setEditing({...editing,language:e.target.value})}>{languages.map(([value,label])=><MenuItem key={value} value={value}>{label}</MenuItem>)}</TextField></Stack>
    ...
    </Stack></Box></DialogContent>}<DialogActions><Button disabled={busy} onClick={()=>setEditing(null)}>Cancel</Button><Button disabled={busy} variant="contained" onClick={save}>{busy?'Saving…':'Save changes'}</Button></DialogActions></Dialog>
    ...
    <Snackbar open={savedNotice} autoHideDuration={3000} onClose={()=>setSavedNotice(false)}><Alert severity="success" variant="filled">Changes saved</Alert></Snackbar>
    ...
    <Box sx={{display:'flex',gap:1.25,overflowX:'auto',py:1.25,pb:1.75}}>
      {Array.from({length:item.quantity},(_,index)=>{
        ...
        return <Box key={index} role="button" aria-label={`${locked?'Assigned':'Select'} copy ${index+1}`} onClick={()=>!locked&&onToggle(index)} sx={{position:'relative',flex:'0 0 92px',p:.75,border:'2px solid',borderColor:checked?'primary.main':'divider',borderRadius:2,cursor:locked?'not-allowed':'pointer',opacity:locked ? .48 : 1,bgcolor:checked?'rgba(255,96,112,.10)':'background.default'}}>
          {item.image_url?<FoilArtwork src={artworkUrl(item.image_url)} alt={`${item.card_name} copy ${index+1}`} foil={item.foil} sx={{width:'100%',aspectRatio:'63 / 88',borderRadius:1,overflow:'hidden'}} imageSx={{objectFit:'contain'}}/>:<Box sx={{width:'100%',aspectRatio:'63 / 88',bgcolor:'action.hover',borderRadius:1}}/>}
