...
import {
  ...
  Skeleton, TablePagination, TextField, Tooltip, Typography,
} from '@mui/material'
...
export default function Collection(){
  ...
  const save=async()=>{if(!editing)return;setBusy(true);setError(undefined);try{await request(`/inventory/${editing.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({language:editing.language,condition:editing.condition,storage_location:editing.storage_location,market_price:editing.market_price,purchase_price:editing.purchase_price,notes:editing.notes})});if(selectedCopies.size&&(copyFoil!==editing.foil||copyCondition!==editing.condition))await request(`/inventory/${editing.id}/move-copies`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({quantity:selectedCopies.size,foil:copyFoil,condition:copyCondition})});setEditing(null);setReload(x=>x+1)}catch(e){setError(e instanceof Error?e.message:'Could not save entry')}finally{setBusy(false)}}
