...
export default function Collection(){
  ...
  return <>
    ...
    <Box className="collection-grid" mt={3}>{items.map(item=><Card key={item.id} sx={{display:'flex',overflow:'hidden',position:'relative',alignItems:'flex-start',minHeight:190}}>
      {item.image_url?<FoilArtwork src={item.image_url} alt={item.card_name} foil={item.foil} sx={{width:120,aspectRatio:'63 / 88',m:1.25,borderRadius:1.5}} imageSx={{objectFit:'contain',objectPosition:'center'}}/>:<Box sx={{width:120,aspectRatio:'63 / 88',m:1.25,bgcolor:'action.hover',borderRadius:1.5}}/>}
