...
const displayPrice=(candidate:Candidate,foil:boolean)=>Number((foil?(candidate.foil_market_price||candidate.market_price):candidate.market_price)||0).toFixed(2)

export default function ReviewQueue(){
  const [items,setItems]=useState<Review[]>([]),[decks,setDecks]=useState<Deck[]>([]),[manual,setManual]=useState<Review|null>(null),[query,setQuery]=useState(''),[matches,setMatches]=useState<Candidate[]>([]),[error,setError]=useState<string>()
  ...
  return <>
    ...
    <Grid container spacing={2}>{items.map(item=><Grid size={{xs:12,lg:6}} key={item.id}><Card><CardContent>
      ...
      <Stack direction={{xs:'column',sm:'row'}} spacing={2} mt={2}><Box component="img" src={`${API}/api/reviews/${item.id}/image`} sx={{width:{xs:'100%',sm:180},maxHeight:260,objectFit:'contain',bgcolor:'#050807',borderRadius:1}}/><Box flex={1} minWidth={0}><Stack direction="row" spacing={.75} flexWrap="wrap" mb={1}><Chip size="small" label={item.defaults.condition.replaceAll('_',' ')}/>{item.defaults.foil&&<Chip size="small" color="warning" label="Foil"/>}<Chip size="small" label={`Deck · ${decks.find(deck=>deck.id===item.defaults.deck_id)?.name||'None'}`}/><Chip size="small" label={`Storage · ${item.defaults.storage_location}`}/></Stack><Typography variant="caption" color="text.secondary" sx={{whiteSpace:'pre-line'}}>OCR: {item.ocr_text||'No readable text'}</Typography>{item.candidates.map(c=><Stack key={c.scryfall_id} direction="row" spacing={1} alignItems="center" mt={1}><Box component="img" src={c.image_url||''} sx={{width:42}}/><Box flex={1}><Typography fontWeight={700}>{c.name}</Typography><Typography variant="body2">{c.set_name} #{c.collector_number}</Typography></Box><Button variant="contained" size="small" onClick={()=>resolve(item,c)}>Choose</Button></Stack>)}<Button startIcon={<Search/>} sx={{mt:2}} onClick={()=>{setManual(item);setQuery('');setMatches([])}}>Search manually</Button></Box></Stack>
    </CardContent></Card></Grid>)}</Grid>
    {!items.length&&<Typography color="text.secondary" textAlign="center" mt={8}>Nothing needs review.</Typography>}
    <Dialog open={!!manual} onClose={()=>setManual(null)} fullWidth maxWidth="md"><DialogTitle>Find the exact printing</DialogTitle><DialogContent><Stack direction="row" spacing={1} mt={1}><TextField autoFocus fullWidth placeholder="Card name" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&query.length>=2)void search()}} slotProps={{input:{startAdornment:<InputAdornment position="start"><Search/></InputAdornment>}}}/><Button variant="contained" disabled={query.length<2} onClick={search}>Search</Button></Stack>{error&&<Alert severity="error" sx={{mt:2}}>{error}</Alert>}<Grid container spacing={2} mt={1}>{matches.map(c=><Grid size={{xs:12,sm:6}} key={c.scryfall_id}><Card variant="outlined"><CardContent><Stack direction="row" spacing={2}><Box component="img" src={c.image_url||''} sx={{width:75}}/><Box flex={1}><Typography fontWeight={800}>{c.name}</Typography><Typography variant="body2">{c.set_name} #{c.collector_number}</Typography><Typography color="primary.main">${displayPrice(c,manual?.defaults.foil||false)}{manual?.defaults.foil?' · foil':''}</Typography><Button size="small" variant="contained" sx={{mt:1}} onClick={()=>manual&&resolve(manual,c)}>Choose printing</Button></Box></Stack></CardContent></Card></Grid>)}</Grid></DialogContent></Dialog>
  </>
}
