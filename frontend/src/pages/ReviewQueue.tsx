...
export default function ReviewQueue(){
  ...
  return <>
    ...
    <Grid container spacing={2}>{items.map(item=><Grid size={{xs:12,lg:6}} key={item.id}><Card><CardContent>
      ...
      <Stack direction={{xs:'column',sm:'row'}} spacing={2} mt={2}><Box component="img" src={`${API}/api/reviews/${item.id}/image`} sx={{width:{xs:'100%',sm:180},maxHeight:260,objectFit:'contain',bgcolor:'#050807',borderRadius:1}}/><Box flex={1} minWidth={0}><Stack direction="row" spacing={.75} flexWrap="wrap" mb={1}><Chip size="small" label={item.defaults.condition.replaceAll('_',' ')}/><Chip size="small" label={item.defaults.language.toUpperCase()}/>{item.defaults.foil&&<Chip size="small" color="warning" label="Foil"/>}<Chip size="small" label={`Deck · ${decks.find(deck=>deck.id===item.defaults.deck_id)?.name||'None'}`}/><Chip size="small" label={`Storage · ${item.defaults.storage_location}`}/></Stack><Typography variant="caption" color="text.secondary" sx={{whiteSpace:'pre-line'}}>OCR: {item.ocr_text||'No readable text'}</Typography>{item.candidates.map(c=><Stack key={c.scryfall_id} direction="row" spacing={1} alignItems="center" mt={1}><Box component="img" src={c.image_url||''} sx={{width:42}}/><Box flex={1}><Typography fontWeight={700}>{c.name}</Typography><Typography variant="body2">{c.set_name} #{c.collector_number} · {c.language.toUpperCase()}</Typography></Box><Button variant="contained" size="small" onClick={()=>resolve(item,c)}>Choose</Button></Stack>)}<Button startIcon={<Search/>} sx={{mt:2}} onClick={()=>{setManual(item);setQuery('');setMatches([])}}>Search manually</Button></Box></Stack>
