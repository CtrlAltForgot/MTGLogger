...
export default function App(){
  ...
  return <ThemeProvider theme={theme}><CssBaseline/><CardDetailsProvider>
    <AppBar position="sticky"><Toolbar sx={{position:'relative',minHeight:{xs:54,md:64},px:{xs:1.25,md:2},gap:{xs:.75,md:1.5}}}>
      ...
      <Tabs value={page} onChange={(_,value)=>changePage(value)} variant="scrollable" scrollButtons={false} aria-label="Main navigation" sx={{flex:{xs:1,lg:'none'},position:{lg:'absolute'},left:{lg:'50%'},transform:{lg:'translateX(-50%)'},width:{lg:'min(70vw, 900px)'},minWidth:0,maxWidth:{md:900},mx:{xs:'auto',lg:0},'& .MuiTabs-flexContainer':{justifyContent:{lg:'center'}},'& .MuiTab-root':{minWidth:0,minHeight:{xs:54,md:64},fontSize:{xs:'.78rem',md:'.95rem'},px:{xs:1,md:1.65},gap:{xs:.35,md:.7}},'& .MuiSvgIcon-root':{fontSize:{xs:21,md:25}}}}>{pages.map(item=><Tab key={item.name} icon={item.icon} iconPosition="start" label={item.name}/>)}</Tabs>
